			 ...
			 iterator_t *it)
{
	...
	double delta =
	    cur_time - intrnl->last_now; // time since the last time we updated
...
static void log_drop_warnings(export_status_t *exp)
{
	if (exp->pcap_drop_last / exp->recv_rate > 0.05) {
		log_warn("monitor",
			 "Dropped %.0f packets in the last second, (%u "
			 "total dropped (pcap: %u + iface: %u))",
...
static inline void check_max_sendto_failures(export_status_t *exp)
{
	if (zconf.max_sendto_failures >= 0 &&
		...
		log_fatal("monitor",
			  "maxiumum number of sendto failures (%i) exceeded",
