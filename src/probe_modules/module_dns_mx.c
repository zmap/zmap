...
// HEADER 12 bytes
// \xd2\x8c -> TransactionID
// \x01\x20 -> Flags: 0x Standard query
// \x00\x01 -> Questions: 1
...
// \x05\x67\x6d\x61\x69\x6c\x03\x63\x6f\x6d\00
// TAILER 4 bytes
// \x00\x0f -> Type: MX (Mail Exchange)
// \x00\x01 -> Class: IN (0x0001)

//default sends to gmail.com
static const char dns_msg_default[32] = "\xd2\x8c\x01\x20\x00\x01\x00\x00\x00\x05\x67\x6d\x69\x6c\x03\x63\x6f\x6d\x00\x00\x0f\x00\x01";
static const char dns_msg_default_head[DNS_HEAD_LEN] = "\xd2\x8c\x01\x20\x00\x01\x00\x01\x00\x00\x00\x00\x00\x00";
// static const char udp_dns_msg_default_name [16] = "\x03\x77\x77\x77\x06\x67\x6f\x6f\x67\x6c\x65\x03\x63\x6f\x6d\x00";
static const char dns_msg_default_tail[DNS_TAIL_LEN]  = "\x00\x0f\x00\x01";
...
static int dns_global_initialize(struct state_conf *conf) {
	...
	udp_send_msg_len = sizeof(dns_msg_default);
	udp_send_msg = malloc(udp_send_msg_len);
	memcpy(udp_send_msg, dns_msg_default, udp_send_msg_len);
	...
	*c++ = 0;
	if (strcmp(args, "name") == 0) {
		...
		convert_to_dns_mx_name_format(dns_domain, (unsigned char*)c);

		udp_send_msg_len=dns_domain_len + DNS_HEAD_LEN + DNS_TAIL_LEN; // domain length +  header + tailer
		udp_send_msg = malloc(udp_send_msg_len);

		// create query packet
		memcpy(udp_send_msg, dns_msg_default_head, sizeof(dns_msg_default_head)); // header
		// random Transaction ID
		random_bytes(udp_send_msg, 2);
		memcpy(udp_send_msg + sizeof(dns_msg_default_head), dns_domain, dns_domain_len); // domain
		memcpy(udp_send_msg + sizeof(dns_msg_default_head) + dns_domain_len, dns_msg_default_tail, sizeof(dns_msg_default_tail)); // trailer
		free(dns_domain);
	} else {
		log_fatal("udp_dns", "unknown UDP DNS probe specification (expected "
				"domain name with name:www.domain.com)");
...
int dns_global_cleanup(__attribute__((unused)) struct state_conf *zconf,
