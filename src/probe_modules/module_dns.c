...
static int build_global_dns_packet(const char *domain)
{
	...
	// All other header fields should be 0. Except id, which we set per
	// thread.
	...
	// Set the qclass to The Internet (TM) (R) (I hope you're happy now
	// Zakir)
	tail_p->qclass =
				...
				uint16_t recursion_level)
{
	...
	if (data_len == 0 || name_len == 0 || payload_len == 0) {
		log_trace("dns",
			  "_get_name_helper OUT, err. 0 length field. datalen "
			  "%d namelen %d payloadlen %d",
	...
	while (data_len > 0) {
		...
		if (byte >= 0xc0) {
			...
			log_trace(
			    "dns",
			    "_get_name_helper OUT. rec level %d success. %d "
			    "bytes consumed.",
			...
			// iteration.
			// Do we have enough data left (must have null byte
			// too)?
			if ((byte + 1) > data_len) {
				log_trace(
				    "dns",
...
static int dns_global_initialize(struct state_conf *conf)
{
	...
	if (!conf->probe_args) { // no parameters passed in. Use defaults
		...
		if (probe_arg_delimiter_p == NULL ||
		    ...
		    conf->probe_args + strlen(conf->probe_args) ==
			probe_arg_delimiter_p + 1) {
			log_fatal(
			    "dns",
			    "Invalid probe args. Format: \"A,google.com\"");
		}
		domain = probe_arg_delimiter_p + 1;
		qtype_str =
		    xmalloc(probe_arg_delimiter_p - conf->probe_args + 1);
		       ...
		       UNUSED port_h_t dst_port, UNUSED void **arg_ptr)
{
	...
	uint16_t len =
	    htons(sizeof(struct ip) + sizeof(struct udphdr) + dns_packet_len);
			...
			uint32_t *validation)
{
	...
	if (ip_hdr->ip_p == IPPROTO_UDP) {
		...
		if (dns_header_p->id == (validation[2] & 0xFFFF)) {
			...
			tail_p = (dns_question_tail *)(dns_packet +
						       sizeof(dns_header) +
						       qname_len);
			// Verify our qname
			if (strcmp(qname, qname_p) == 0) {
				// Verify the qtype and qclass.
				if (tail_p->qtype == htons(qtype) &&
		...
		fs_add_bool(fs, "app_success", is_valid &&
						   (qr == DNS_QR_ANSWER) &&
...
probe_module_t module_dns = {
	...
	"supports sending the the following types: of queries: A, NS, CNAME, "
	"SOA, "
	"PTR, MX, TXT, AAAA, RRSIG, and ALL. The module will accept and "
	"attempt "
	"to parse all DNS responses. There is currently support for parsing "
	"out "
	"full data from A, NS, CNAME, MX, TXT, and AAAA. Any other types will "
	"be "
	"output in raw form."
