...
static const char udp_send_msg_default[192] =	"\x17\x00\x03\x2a\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"/
...
void udp_ntpmonlist_process_packet(const u_char *packet, UNUSED uint32_t len, fieldset_t *fs)
{
	...
	if (ip_hdr->ip_p == IPPROTO_UDP) {
		...
		app_success =   (  (payload[0]==0xd7) && (payload[1]==0x00) && (payload[2]==0x03) && (payload[3]==0x2a) );
		...
		fs_add_string(fs, "app_rstr", (char *) udp_ntp_monlist_response_strings[app_success], 0);
...
static fielddef_t fields[] = {
	...
	{.name = "app_rstr", .type = "string", .desc = "for udp_ntpmonlist module: NTP ERR invalid monlist response - NTP NOERR valid monlist response"},
