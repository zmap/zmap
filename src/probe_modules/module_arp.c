...
void arp_print_packet(FILE *fp, const void* packet)
{
...
int arp_validate_packet(const void *packet,
		uint32_t len, __attribute__((unused)) uint32_t *src_ip, __attribute__((unused)) uint32_t *validation)
{
	struct arp_pkt *arp_packet = (struct arp_pkt *) &((struct ether_header *)packet)[1];
	uint32_t arp_len = len - sizeof(struct ether_header);

	if (sizeof(arp_packet) > arp_len) {
		return 0;
	}
