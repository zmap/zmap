...
/* send module for performing massive UDP Open CHARGEN scans
 * default will send random <CHARGEN_MSG_DEFAULT_LEN> byte query
 * SUCCESS only for response that start with at least 5 bytes of chargen sequence */
...
#define CHARGEN_MSG_DEFAULT_LEN 1;

/* defined in module_udp.c
const char *udp_unreach_strings[] */

const char *udp_chargen_response_strings[] = {
        "CHARGEN ERR invalid sequence",
        "CHARGEN NOERR valid sequence"
};

static int num_ports;

probe_module_t module_udp_chargen;
...
static int udp_chargen_global_initialize(struct state_conf *conf) {
	char *args, *c;
	int i;
	unsigned int n;
	FILE *inp;
	...
	udp_send_msg_len = CHARGEN_MSG_DEFAULT_LEN;
	udp_send_msg = malloc(udp_send_msg_len);
	random_bytes(udp_send_msg, udp_send_msg_len);
	...
	if (! c) {
		free(args);
		free(udp_send_msg);
		log_fatal("udp_chargen", "unknown UDP CHARGEN probe specification (expected "
	...
	*c++ = 0;
	...
	if (strcmp(args, "text") == 0) {
		...
		if (!inp) {
			free(args);
			free(udp_send_msg);
			log_fatal("udp_chargen", "could not open UDP CHARGEN data file '%s'\n", c);
			exit(1);
		}
		free(udp_send_msg);
		...
		if (! udp_send_msg) {
			free(args);
			log_fatal("udp_chargen", "failed to malloc payload buffer");
			exit(1);
		}
		...
		for (i=0; i < udp_send_msg_len; i++) {
			if (sscanf(c + (i*2), "%2x", &n) != 1) {
				free(args);
				free(udp_send_msg);
				log_fatal("udp_chargen", "non-hex character: '%c'", c[i*2]);
				exit(1);
			}
	...
	if (udp_send_msg_len > MAX_UDP_PAYLOAD_LEN) {
		log_warn("udp_chargen", "warning: reducing UDP payload to %d "
...
int udp_chargen_global_cleanup(__attribute__((unused)) struct state_conf *zconf,
...
int udp_chargen_init_perthread(void* buf, macaddr_t *src,
		macaddr_t *gw, __attribute__((unused)) port_h_t dst_port)
{
	...
	module_udp_chargen.packet_length = sizeof(struct ether_header) + sizeof(struct ip) 
				+ sizeof(struct udphdr) + udp_send_msg_len;
	assert(module_udp_chargen.packet_length <= MAX_PACKET_SIZE);
...
void udp_chargen_print_packet(FILE *fp, void* packet)
{
	...
	fprintf(fp, "udp_chargen { source: %u | dest: %u | checksum: %u }\n",
		ntohs(udph->uh_sport),
		ntohs(udph->uh_dport),
		ntohl(udph->uh_sum));
...
void udp_chargen_process_packet(const u_char *packet, UNUSED uint32_t len, fieldset_t *fs)
{
	int app_success;
	// log_debug("udp_chargen", "chargen_process_packet");
                ...
                __attribute__((unused))uint32_t *src_ip, uint32_t *validation)
{
	...
	// log_debug("udp_chargen", "chargen_validate_packet");
        if (ip_hdr->ip_p == IPPROTO_UDP) {
...
probe_module_t module_udp_chargen = {
	...
	.port_args = 1,
	.thread_initialize = &udp_chargen_init_perthread,
	.global_initialize = &udp_chargen_global_initialize,
	.make_packet = &udp_chargen_make_packet,
	.print_packet = &udp_chargen_print_packet,
	.validate_packet = &udp_chargen_validate_packet,
	.process_packet = &udp_chargen_process_packet,
	.close = &udp_chargen_global_cleanup,
	.fields = fields,
