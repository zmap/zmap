...
static void mongodb_module_log(mongoc_log_level_t log_level, const char *log_domain,
...
static void append_fs_to_bson(bson_t *doc, const char *name, fieldset_t *fs){
    bson_t inner;
    if (fs->len > 0) {
        bson_append_document_begin(doc, name, (int) strlen (name), &inner);
...
static void append_repeated_to_bson(bson_t *doc, const char *name, fieldset_t *fs){
    bson_t inner;
    char str[3];
    if (fs->len > 0){
        const char *key;
