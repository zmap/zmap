...
int mongodb_module_process(fieldset_t *fs)
{
	...
	for (int i = 0; i < fs->len; i++) {
		...
		if (f->type == FS_STRING) {
			...
			BSON_APPEND_BINARY(doc, f->name, BSON_SUBTYPE_BINARY,
					   f->value.ptr, f->len);
...
output_module_t module_mongodb = {
    ...
    .helptext = "Write output to MongoDB. Defaults to "
		"mongodb://localhost:27017/zmap_output. Specify a custom "
		"connection URI in output module args."};
