			...
			UNUSED int fieldlens)
{
	...
	log_info("redis-csv",
		 "the following fields will be output to redis: %s.",
		 fieldstring);
	free(fields);
...
{
	if (redis_lpush_strings(rctx, (char *)queue_name, buffer,
				buffer_fill)) {
		return EXIT_FAILURE;
	}
...
output_module_t module_redis_csv = {
    ...
    .helptext = "Outputs one or more output fields in csv, and then flushes "
		"out to redis. \n"
		"By default, the probe module does not filter out duplicates "
		"or limit to successful fields, \n"
		"but rather includes all received packets. Fields can be "
		"controlled by \n"
		"setting --output-fileds. Filtering out failures and duplicate "
		"packets can \n"
