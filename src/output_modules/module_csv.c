...
output_module_t module_csv_file = {
    ...
    .helptext =
	"Outputs one or more output fields as a comma-delimited file. By "
	"default, the "
	"probe module does not filter out duplicates or limit to successful "
	"fields, "
	"but rather includes all received packets. Fields can be controlled by "
	"setting --output-fields. Filtering out failures and duplicate packets "
	"can "
