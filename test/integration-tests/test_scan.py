...
def test_scan_with_liburing():
    ...
    with open(output_file_path, 'w') as file:
        file.write("")
    # using known dns resolvers to make this test deterministic
    t = zmap_wrapper.Wrapper(dryrun=False, port=53, subnet=" ".join(known_active_ips), threads=1, output_file=output_file_path, max_cooldown=3, enable_liburing=True)
    t.run()
    # read the file into a list of IPs
    ips = []
    with open(output_file_path, 'r') as file:
